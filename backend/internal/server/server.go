...
func New(opts Options) http.Handler {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.With(authRoute).Route("/instances", func(r chi.Router) {
			...
			r.Get("/{id}/working/stats", instH.WorkingStats)
			r.Get("/{id}/working/sessions", instH.WorkingListSessions)
			r.Get("/{id}/working/sessions/{sessionId}/keys", instH.WorkingListKeys)
			r.Put("/{id}/working/sessions/{sessionId}/keys/{key}", instH.WorkingPutKey)
			r.Delete("/{id}/working/sessions/{sessionId}/keys/{key}", instH.WorkingDeleteKey)
			r.Delete("/{id}/working/sessions/{sessionId}", instH.WorkingDeleteSession)
			r.Post("/{id}/working/flush-expired", instH.WorkingFlushExpired)
			r.Get("/{id}", instH.Get)
